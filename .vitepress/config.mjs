...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    themeConfig: {
      ...
      socialLinks: [
        { icon: 'github', link: 'https://github.com/wyrzmz' }
      ]
    },

    // 路由重定向
    rewrites: {
      'views/examples/:folder/(.*)': 'examples/:folder/(.*)' // :folder 是一个路径参数，表示文件夹名称。* 是一个通配符，它会匹配 views/examples/:folder/ 文件夹中的任何文件，无论文件名是什么。
      // 可以写成如下形式，表示直接重定向views下所有的文件
      // '^views/(.*)$': '(.*)', // 匹配 views 下所有文件并重定向
    },
  }
})
